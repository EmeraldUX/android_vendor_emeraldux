...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the EmeraldUX model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch emeraldux_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/EmeraldUX-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
...
function repopick() {
    T=$(gettop)
    $T/vendor/emeraldux/build/tools/repopick.py $@
}
